...
if ($f == "add-new-forum" && Wo_IsAdmin() == true && Wo_CheckSession($hash_id) === true) {
    if (empty($_POST['name']) || empty($_POST['description']) || empty($_POST['section'])) {
        ...
        if (strlen($_POST['name']) < 5) {
            $error = $error_icon . $wo['lang']['name_more_than4'];
        }
        if (strlen($_POST['name']) > 100) {
            $error = $error_icon . $wo['lang']['name_less_than100'];
        }
        if (strlen($_POST['description']) < 5) {
            $error = $error_icon . $wo['lang']['desc_more_than4'];
        }
        if (strlen($_POST['description']) > 190) {
            $error = $error_icon . $wo['lang']['desc_less_than190'];
        }
    }
    if (empty($error)) {
