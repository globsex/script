...
if ($f == "yoomoney") {
	if ($s == 'create') {
		if (!empty($_GET['amount']) && is_numeric($_GET['amount']) && $_GET['amount'] > 0) {
			...
			$successURL = $wo['config']['site_url'] . "/requests.php?f=yoomoney&s=success&amount=".$amount;
			if (!empty($wo["config"]['currency_array']) && in_array('RUB', $wo["config"]['currency_array']) && 'RUB' != $wo['config']['currency'] && !empty($wo['config']['exchange']) && !empty($wo['config']['exchange']['RUB'])) {
                $amount = (($_GET['amount'] * $wo['config']['exchange']['RUB']));
            }
