...
function Wo_FeaturedUsers($limit = '', $type = '', $preferred_genders = array()) {
    global $wo, $sqlConnect;
    $pro_types = array(
        1,
        2,
    ...
    if (!empty($preferred_genders)) {
        $preferred_genders = implode("','", $preferred_genders);
        $gender_condition = " AND `gender` IN ('{$preferred_genders}')";
    }

    $query_one = "SELECT `user_id` FROM " . T_USERS . " WHERE `active` = '1' AND `user_id` NOT IN (SELECT `blocked` FROM " . T_BLOCKS . " WHERE `blocker` = '{$logged_user_id}') AND `user_id` NOT IN (SELECT `blocker` FROM " . T_BLOCKS . " WHERE `blocked` = '{$logged_user_id}') AND `is_pro` = '1' {$type_text} {$gender_condition} ORDER BY RAND() LIMIT {$limit}";
    $sql = mysqli_query($sqlConnect, $query_one);
...
function Wo_UserSug($limit = 20) {
    ...
    $logged_user_gender = $wo['user']['gender'];

    $logged_user_address = $wo['user']['address'];
    ...
    switch ($logged_user_gender) {

        case 'male':

            $target_genders = ['female', 'transex'];
        ...
        case '1651': // Casal Ele/Ele

            $target_genders = ['gay'];

            break;

        case '1652': // Casal Ele/Ela

            $target_genders = ['female', 'male'];

            break;

        case '1653': // Casal Ela/Ela
    ...
    if (empty($target_genders)) {
        ...
        return $data; // No target gender specified

    }



    $genders_string = "'" . implode("', '", $target_genders) . "'";



    // Prioritize users in the same city

    $query_one = "SELECT `user_id` FROM " . T_USERS . " WHERE `active` = '1' AND `gender` IN ($genders_string) AND `address` = '$logged_user_address' AND `user_id` NOT IN (SELECT `blocked` FROM " . T_BLOCKS . " WHERE `blocker` = '{$user_id}') AND `user_id` NOT IN (SELECT `blocker` FROM " . T_BLOCKS . " WHERE `blocked` = '{$user_id}') AND `user_id` NOT IN (SELECT `following_id` FROM " . T_FOLLOWERS . " WHERE `follower_id` = {$user_id}) AND `user_id` <> {$user_id}";
    ...
    if (mysqli_num_rows($sql) < $limit) {

        // If not enough users found in the same city, search more users ignoring the city

        $remaining_limit = $limit - mysqli_num_rows($sql);

        $query_two = "SELECT `user_id` FROM " . T_USERS . " WHERE `active` = '1' AND `gender` IN ($genders_string) AND `user_id` NOT IN (SELECT `blocked` FROM " . T_BLOCKS . " WHERE `blocker` = '{$user_id}') AND `user_id` NOT IN (SELECT `blocker` FROM " . T_BLOCKS . " WHERE `blocked` = '{$user_id}') AND `user_id` NOT IN (SELECT `following_id` FROM " . T_FOLLOWERS . " WHERE `follower_id` = {$user_id}) AND `user_id` <> {$user_id} AND `user_id` NOT IN (SELECT `user_id` FROM (" . $query_one . ") AS already_selected)";

        $query_two .= " ORDER BY RAND() LIMIT {$remaining_limit}";



        $sql = mysqli_query($sqlConnect, $query_two);
            ...
            if ($wo['config']['emailNotification'] == 1 && $recipient['emailNotification'] == 1) {
                ...
                if ($send_mail == true) {
                    ...
                    $send_message_data = array(
                        ...
                        'subject' => 'Nova atividade em globsex.com.br',
...

{
    ...
    $query = " SELECT `user_id` FROM " . T_USERS . " WHERE (`username` LIKE '%{$query}%' OR CONCAT( `first_name`,  ' ', `last_name`,  ' ', `address` ) LIKE  '%{$query}%') {$job_type_main} {$workplaces_type_main} {$custom_query}";
    ...
    if (!empty($limit)) {
        ...
        $query .= " ORDER BY `is_pro` DESC, `user_id` DESC LIMIT {$limit}";
...

{
    ...
    // 🔍 INÍCIO DO FILTRO POR GÊNERO E CIDADE (baseado no Wo_UserSug)
    if ($logged_user_id) {
        $user_data = Wo_UserData($logged_user_id);
        $logged_user_gender = $user_data['gender'];
        $logged_user_address = Wo_Secure($user_data['address']);
        $target_genders = array();
        ...
        if (!empty($target_genders)) {
            $genders_string = "'" . implode("','", array_map('Wo_Secure', $target_genders)) . "'";
            $query_text .= " AND `user_id` IN (
                SELECT `user_id` FROM " . T_USERS . "
                WHERE `gender` IN ({$genders_string}) AND `address` = '{$logged_user_address}'
                
               UNION

               SELECT `following_id` FROM " . T_FOLLOWERS . "
               WHERE `follower_id` = {$logged_user_id} AND `active` = '1'
            )";
        }
    }
    // 🔍 FIM DO FILTRO POR GÊNERO E CIDADE
        if ($logged_user_id) {
