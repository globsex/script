...
function Wo_PageSug($limit = 1, $page_id = 0, $type = "next") {
    ...
    $data = array();
    $user_id   = Wo_Secure($wo["user"]["user_id"]);
    $user_city = Wo_Secure($wo["user"]["address"]); // cidade do usuário logado
...
function Wo_GetUserProType($user_type = 0, $type = "post") {
    ...
    $data = array();
    $data["type_name"] = $wo["lang"]["member"] . " " . $wo["pro_packages"][$pro_type]['name'];
    if ($type == "post") {
...
function Wo_GetPromotedPost() {
    ...
    $type_table     = T_POSTS;
    $logged_user_id = Wo_Secure($wo["user"]["user_id"]);
    $logged_user_gender = Wo_Secure($wo["user"]["gender"]);

    // Definindo a lógica para gêneros
    $gender_preferences = array(
        'female' => array('male'),
        'male' => array('female', 'transex'),
        '1650' => array('male', '2805'), // Transex
        '1651' => array('1651', '2805', 'male'), // Casal Ele/Ele
        '1652' => array('female', '1650', '1652', '1653'), // Casal Ele/Ela
        '1653' => array('female', '1653'), // Casal Ela/Ela
        '2805' => array('2805', 'male', '1651') // Gay
    );

    $opposite_genders = isset($gender_preferences[$logged_user_gender]) ? $gender_preferences[$logged_user_gender] : array();

    if (empty($opposite_genders)) {
        return array();
    }

    $opposite_genders_list = "'" . implode("', '", $opposite_genders) . "'";

    $query_one = mysqli_query($sqlConnect, "SELECT `id` FROM {$type_table} 
                                            WHERE `boosted` = '1' 
                                            ...
                                            AND `user_id` IN (SELECT `user_id` FROM " . T_USERS . " WHERE `gender` IN ({$opposite_genders_list}))
...
function Wo_GetPromotedPage() {
    ...
    $data           = array();
    $logged_user_id = Wo_Secure($wo["user"]["user_id"]);
    $user_city      = Wo_Secure($wo["user"]["address"]);

    // Primeiro tenta pegar boosted na mesma cidade
    $query_one = "
        SELECT `page_id` 
        FROM {$type_table} 
        WHERE `boosted` = '1' 
          AND `address` = '{$user_city}'
          AND `page_id` NOT IN (
              SELECT `page_id` FROM " . T_PAGES . " 
              WHERE `user_id` = {$logged_user_id} AND `active` = '1'
          ) 
          AND `page_id` NOT IN (
    ...
    // Se não achar nenhuma página na cidade, pega páginas boosted aleatórias
    if (empty($data)) {
        $query_two = "
            SELECT `page_id` 
            FROM {$type_table} 
        ...
        $sql_two = mysqli_query($sqlConnect, $query_two);

        if (mysqli_num_rows($sql_two)) {
            while ($fetched_data = mysqli_fetch_assoc($sql_two)) {
                $data[] = Wo_PageData($fetched_data["page_id"]);
            }
        }
    }
...
function Wo_GetBoostedPages($user_id) {
    ...
    $user_id = Wo_Secure($user_id);
    $query_user = mysqli_query($sqlConnect, "SELECT `address` FROM " . T_USERS . " WHERE `user_id` = '{$user_id}' LIMIT 1");
    $user_address = '';
    if (mysqli_num_rows($query_user) == 1) {
        $user_data = mysqli_fetch_assoc($query_user);
        $user_address = Wo_Secure($user_data['address']);
    }

    if (!empty($user_address)) {
        // Tenta buscar páginas impulsionadas na mesma cidade
        $query = mysqli_query($sqlConnect, "SELECT `page_id` FROM " . T_PAGES . " WHERE `user_id` = {$user_id} AND `boosted` = '1' AND `address` = '{$user_address}' ORDER BY `page_id` DESC");
    ...
    // Se não encontrou nenhuma página na cidade do usuário, busca páginas aleatórias
    if (empty($data)) {
        $query_random = mysqli_query($sqlConnect, "SELECT `page_id` FROM " . T_PAGES . " WHERE `user_id` = {$user_id} AND `boosted` = '1' ORDER BY RAND() LIMIT 10");
        if (mysqli_num_rows($query_random)) {
            while ($fetched_data = mysqli_fetch_assoc($query_random)) {
                $data[] = Wo_PageData($fetched_data["page_id"]);
            }
        }
    }
